...
if [ "$ROS2_VERSION" = "foxy" ]; then
    # Foxy已EOL，使用官方源（可能仍保留旧包）
    echo "ROS2 Foxy已EOL，尝试使用官方源..."
...
if [ "$ROS2_VERSION" = "foxy" ]; then
    # 对于Foxy，使用--fix-missing和--ignore-missing参数
    sudo apt install -y --fix-missing ros-${ROS2_VERSION}-desktop || {
        echo "完整桌面版安装失败，尝试安装基础版..."
        sudo apt install -y --fix-missing ros-${ROS2_VERSION}-ros-base
    }
else
